...
def threshold_single(coeff_vector_scaled, coeff_vector, optim_config):
    ...
    thresh_val = thresh_func(coeff_vector_scaled, coeff_vector, optim_config)
    
    sparse_coeff_vector = torch.where(torch.abs(coeff_vector_scaled) > thresh_val, coeff_vector, torch.zeros_like(coeff_vector_scaled))
...
def threshold(coeff_vector_list, sparse_theta_list, time_deriv_list, optim_config):
    '''Wrapper around threshold_single to threshold list of vectors. Also performs scaling.'''

    coeff_vector_scaled_list = scaling(coeff_vector_list, sparse_theta_list, time_deriv_list)
    result = [threshold_single(coeff_vector_scaled, coeff_vector, optim_config) for coeff_vector_scaled, coeff_vector in zip(coeff_vector_scaled_list, coeff_vector_list)]
